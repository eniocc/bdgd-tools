...
def get_holidays_br(year):
    holiday_list = list(holidays.Brazil(years=year).items())
...
def count_days(day,month, year):
    if day =='sab':
        ...
        num_days = sum(x[day_to_count] != 0 for x in matrix)
    if day =='dom':
...
def count_day_type(ano):
    ...
    for i in range(12):
        if i < 9:
            df_aux.loc[i,'mes_ano'] = f'{str(ano)}-0{str(i + 1)}'
        else:
            df_aux.loc[i,'mes_ano'] = f'{str(ano)}-{str(i + 1)}'
    ...
    df_days.iloc[-1,-1] = f'{str(ano + 1)}-01'
